...
$(document).ready(function () {
  ...
  function moveCursor(direction) {
    ...
    if (ypos == rows.length - 1) {
      ...
      $(
        "#buttons tbody .manifest:nth-child(" +
          (xpos + 1) +
          "), #buttons tbody .option:nth-child(" +
          (xpos + 1) +
          ")"
      ).addClass("active");
    }
  ...
  function calculateManifestURL(params) {
    ...
    url = url.replace("{stream}", stream).replace("{mode}", mode);
    delete params.mode;
    return url;
  }
  ...
  function updateManifestURL() {
    ...
    if (window.history && typeof history.pushState === "function") {
      params.mpd =
        "mpd=" + $("#buttons tbody .manifest.selected").data("filename");
      url = pageState.baseurl + "#" + Object.values(params).join("&");
      history.replaceState(
        pageState,
        $("#buttons tbody .manifest.selected").text(),
        url
      );
    }
  }
  ...
  function playVideo() {
    ...
    url = url.replace(/^\/dash/, "/es5/play");
    url = url.replace(/^\/mps/, "/es5/play");
  ...
  $("body").on("keydown", function (ev) {
    ...
    switch (ev.keyCode) {
      case 13 /* enter */:
        active = $(
          "#row" + cursor.row + " td:nth-child(" + (cursor.col + 1) + ")"
        );
        if (active.length === 0) {
  ...
  function init() {
    ...
    if (/#/.test(document.location.href)) {
      ...
      for (i = 0; i < params.length; ++i) {
        ...
        if (p[0] == "mpd") {
          $("#buttons .selected").removeClass("selected");
          $('#buttons .manifest[data-filename="' + p[1] + '"]').addClass(
            "selected"
          );
        } else {
